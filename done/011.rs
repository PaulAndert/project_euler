...
pub fn main() {
    ...
    for i in 0..20 {
        for j in 0..20 {
            ...
            // Horizontal | best = 48477312
            if j < 17 {
                current = matrix[i][j] * matrix[i][j+1] * matrix[i][j+2] * matrix[i][j+3];
                println!("{}:{} = {} ({})", i, j, current, best);
                if current > best {
                    best = current;
                }
            }

            // Vertical | best = 51267216
            if i < 17 {
...
// horizontal
            if j < 17 {
                current = matrix[i][j] * matrix[i][j+1] * matrix[i][j+2] * matrix[i][j+3];
                //println!("{} * {} * {} * {} = {}", matrix[i][j], matrix[i][j+1], matrix[i][j+2], matrix[i][j+3], current);
                if current > best { 
                    //println!("H New {} > {}", current, best);
                    best = current 
                }
            }
            ...
            if i < 17 {
                current = matrix[i][j] * matrix[i+1][j] * matrix[i+2][j] * matrix[i+3][j];
                //println!("{} * {} * {} * {} = {}", matrix[i][j], matrix[i+1][j], matrix[i+2][j], matrix[i+3][j], current);
                if current > best { 
                    //println!("V New {} > {}", current, best);
                    best = current 
                }
            }
            ...
            if i < 17 && j < 17 {
                current = matrix[i][j] * matrix[i+1][j+1] * matrix[i+2][j+2] * matrix[i+3][j+3];
                //println!("{} * {} * {} * {} = {}", matrix[i][j], matrix[i+1][j+1], matrix[i+2][j+2], matrix[i+3][j+3], current);
                if current > best { 
                    //println!("DL New {} > {}", current, best);
                    best = current 
                }
            }
